function map(valueIn, minIn, maxIn, minOut, maxOut) {
  ...
  function createGrid(divGrid, qtdeLinhas, qtdeColunas, paramOpcoes) {
    ...
    const atualizarListaPalavras = () => {
      ...
      const divMensagem = document.querySelector('.messagemResult');
      ...
      listaPalavras.sort().forEach(objPalavra => {
        ...
        if(objPalavra.palavraEstaCirculada){
          ...
          if(objPalavra.palavra == "CÓDIGO"){
          divMensagem.innerHTML = "Parabéns, você encontrou a palavra CÓDIGO!";
          }
        } 
  ...
  const divGrid = document.querySelector(".grid-caca-palavras");
  const grid = createGrid(divGrid, 10, 10, { habilitarPalavrasInvertidas: false });
  
  function setup() {
    grid.inicializar();
    const listaPalavras = [
    'TUBARÃO', 'MAR', 'ECOSSISTEMA', 'PEIXES', 'CAÇADOR', 'EQUILÍBRIO', 'NATUREZA', 'ALIMENTO', 'PROTEÇÃO', 'VIDA'
    ];
